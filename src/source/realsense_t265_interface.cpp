...
void RealsenseT265Interface::ownSetUp()
{
	...
	ros_utils_lib::getPrivateParam<std::string>("~realsense_odom_topic" 	    , realsense_odom_topic_ 			,"realsense_t265/odom/sample");
	

	tf2_fix_transforms_.clear();
	tf2_fix_transforms_.emplace_back(getTransformation("map","odom",0,0,0,0,0,0));
	tf2_fix_transforms_.emplace_back(getTransformation("odom","odom_rs",RS_OFFSET_X,RS_OFFSET_Y,RS_OFFSET_Z,0,0,0));
	tf2_fix_transforms_.emplace_back(getTransformation("base_link_rs","base_link_rs_NED",0,0,0,RS_OFFSET_R,RS_OFFSET_P,RS_OFFSET_YAW));
	tf2_fix_transforms_.emplace_back(getTransformation("base_link_rs_NED","base_link",-RS_OFFSET_X,-RS_OFFSET_Y,-RS_OFFSET_Z,0,0,0));
...
void RealsenseT265Interface::ownStart(){

  	odom_sub_  = nh_.subscribe("/" + n_space_ + "/" + realsense_odom_topic_	,1, &RealsenseT265Interface::odometryCallback,this);	
...
void RealsenseT265Interface::ownStop(){
	odom_sub_.shutdown();
	pose_pub_.shutdown();
	twist_pub_.shutdown();
}
...
void RealsenseT265Interface::publishEstimatedPose(){
	...
	try {
		...
		auto linear_twist_rotation_transform = tf2_buffer_.lookupTransform("odom","base_link_rs",ros::Time(0));
		...
		tf2::doTransform(vio_twist_linear_vect, odom_twist_linear_vect,linear_twist_rotation_transform);
		odom_twist.twist.linear = odom_twist_linear_vect;

		// obtain mav angular speed in base_link frame 
		auto angular_twist_rotation_transform = tf2_buffer_.lookupTransform("base_link_rs_NED","base_link_rs",ros::Time(0));
		tf2::doTransform(vio_twist_angular_vect, base_link_twist_angular_vect, angular_twist_rotation_transform);
